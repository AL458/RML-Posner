...
for subj = 1:length(subj_array)
    ...
    while 1 
        ...
        % Get next location in parameter space, and the corresponding MMSE
        [loc_next,MMSE_next,param] = optimize_gradient_MMSE(param,grad,loc,MMSE);
        % Test for convergence, and whether the desired number of
        % iterations is met
        ...
        % Update the current location, its MMSE, and the param structure
        [param,loc,MMSE] = update_vars_MMSE(param,loc_next,MMSE_next);
    end
    % Test which parameter set is optimal
    ...
    % Save the output to the temp folder
